...
class BillingController extends BaseController
{
    ...
    public function printExpenseEntry(Request $request)
    {
     ...
     {  
        ...
        $Invoices = $Invoices->orderBy($columns[$requestData['order'][0]['column']] ?? "invoices.id", $requestData['order'][0]['dir'] ?? 'desc');
    ...
    public function saveInvoicePayment(Request $request)
    {
        ...
        if ($validator->fails())
        {
            ...
            try {
                ...
                $newInvoiceHistory = InvoiceHistory::create([
                    'invoice_id' => $request->invoice_id,
                    ...
                    'invoice_payment_id' => $lastInvoicePaymentId,
                    'notes' => $request->notes,
                    'status' => "1",
                    'created_by' => auth()->id(),
                    'deposit_into' => "Operating Account", // This means payment by which client
                    'deposit_into_id' => $request->contact_id, // This means payment by which client
                ]);
                ...
                $invoiceHistory['created_at']=date('Y-m-d H:i:s');
                $newHistoryId = $this->invoiceHistory($invoiceHistory); */
                $request->request->add(["invoice_history_id" => $newInvoiceHistory->id]);
            ...
            foreach($invoiceIds as $id) {
                ...
                if($invoiceHistories) {
                    foreach($invoiceHistories as $key => $item) {
                        if($item->refund_ref_id == '' && $item->status == '1' && $item->deposit_into_id != '') {
                ...
                $this->deleteInvoiceFundTransfer($Invoices);
    ...
    public function saveRefundPopup(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            try {
                ...
                if($invoiceHistory->payment_from == "offline" && !in_array($invoiceHistory->pay_method, ["Trust", "Non-Trust Credit Account", "Credit Account"])) {
                    //Insert invoice payment record.
                    $entryDone = InvoicePayment::create([
                        'invoice_id'=>$findInvoice['id'],
                        'payment_from'=>'offline',
                ...
                } else if($invoiceHistory->payment_from == "credit" && in_array($invoiceHistory->pay_method, ["Non-Trust Credit Account","Credit Account"])) {
                    //Insert invoice payment record.
                    ...
                    $invoiceHistoryNew['invoice_payment_id']=$entryDone->id;
                    $invoiceHistoryNew['pay_method']="Credit Refund";
    ...
    public function deletePaymentEntry(Request $request)
    {
        ...
        if ($validator->fails())
        {
            ...
            try {
                ...
                } else if($PaymentMaster->payment_from == "credit" && in_array($PaymentMaster->pay_method, ["Non-Trust Credit Account","Refund","Credit Account"])){
       ...
       $validator = \Validator::make($request->all(), [
        ...
        if ($validator->fails())
        {
            ...
            if(!empty($userData)){
                ...
                $fundType = "credit";
                return view('billing.dashboard.depositNonTrustFundPopup',compact('userData','fundRequestList','request', 'fundType'));
                exit;  
            }else{
                return response()->json(['errors'=>'error']);
            }
    ...
    public function saveDepositIntoNonTrustPopup(Request $request)
    {
        ...
        if ($validator->fails())
        {
            ...
            $DepositIntoCreditHistory->firm_id=$authUser->firm_name;                
            $DepositIntoCreditHistory->related_to_fund_request_id = @$refundRequest->id;                
            $DepositIntoCreditHistory->created_by=$authUser->id;                
    ...
    public function recordPayment(Request $request)
    {
       ...
       if ($validator->fails())
       {
        ...
        //  ->where("invoices.status","!=","Paid")
         ->whereNotIn("invoices.status", ["Paid","Forwarded"])
    ...
    public function saveDepositIntoTrustPopup(Request $request)
    {
        ...
        try {
            ...
            $UsersAdditionalInfo=UsersAdditionalInfo::select("trust_account_balance")->where("user_id",$request->trust_account)->first();
            $authUserTimezone = $authUser->user_timezone;
            $TrustInvoice=new TrustHistory;
            $TrustInvoice->client_id=$request->trust_account;
            $TrustInvoice->payment_method=$request->payment_method;
            $TrustInvoice->amount_paid=$request->amount;
            ...
            $TrustInvoice->created_by=$authUser->id; 
    ...
    public function loadSummary(Request $request)
    {
        ...
        $monthTimeEntryData = TaskTimeEntry::select('task_time_entry.*')->whereBetween('entry_date',[$startDate,$endDate]);
        // ->whereIn('created_by',$getChildUsers);
        ...
        $monthTimeEntryData=$monthTimeEntryData
        ...
        $monthTimeEntryDataForCalander=$monthTimeEntryDataForCalander
        ->where('deleted_at',NULL)
        ->groupBy('entry_date')
        ->get(); 
    ...
    public function loadAllSavedTimeEntry(Request $request)
    {
        ...
        if($request->forUser!=0){
            ...
            $TaskTimeEntry = $TaskTimeEntry->where("user_id", auth()->id());
        }
    ...
    /**
     * Trust/Credit/Request fund online payment from lawyer portal, Get credit/debit card detail and do payment, cash payment and bank transfer payment 
     */
    ...
    public function fundCardPayment($request, $client, $customerId)
    {
        ...
        $validOrderWithCharge = [
            ...
            'currency'    => 'MXN',
            'metadata'    => array('payment' => 'Trust/Credit/Request fund card payment')
        ];
        if($request->emi_month == 0) {
            $validOrderWithCharge['charges'] = [
                [
        ...
        if($request->type == 'fund') {            
            $order = \Conekta\Order::create($validOrderWithCharge);                        
            if($order->payment_status == 'paid') { 
                $fundRequest = RequestedFund::whereId($request->applied_to)->where('status', '!=', 'paid')->first();
                if($fundRequest) {
                ...
                if($request->fund_type == "trust") {
                    ...
                    $creditHistory = DepositIntoCreditHistory::create([
                        ...
                        'payment_type' => "deposit",
                        'firm_id' => $client->firm_name,
                        'related_to_fund_request_id' => @$request->applied_to,
                        'created_by' => $authUser->id,
                        'online_payment_status' => $order->payment_status,
                        'notes' => $request->notes,
                    ]);
                ...
                $emailType = ($fundRequest) ? 'fundrequest' : 'fund';
                // Send confirm email to client
                $this->dispatch(new OnlinePaymentEmailJob($fundRequest, $client, $emailTemplateId = 30, $requestOnlinePayment, 'client', $emailType));
    ...
    public function fundCashPayment($request, $client, $customerId)
    {
        ...
        $validOrderWithCharge = [
            ...
            'currency'    => 'MXN',
            'metadata'    => array('payment' => 'Trust/Credit/Request fund cash payment')
        ];
        if($request->type == 'fund') {
            $order = \Conekta\Order::create($validOrderWithCharge);
            if($order->payment_status == 'pending_payment') {
                ...
                // Cash payment reference email to client
                $this->dispatch(new OnlinePaymentEmailJob($fundRequest, $client, $emailTemplateId = 32, $requestOnlinePayment, 'cash_reference_client', ($fundRequest) ? 'fundrequest' : 'fund'));
    ...
    public function fundBankPayment($request, $client, $customerId)
    {
        ...
        $validOrderWithCharge = [
            ...
            'currency'    => 'MXN',
            'metadata'    => array('payment' => 'Trust/Credit/Request fund bank payment')
        ];
        if($request->type == 'fund') {
            $order = \Conekta\Order::create($validOrderWithCharge);
            if($order->payment_status == 'pending_payment') {
                ...
                // Bank payment reference email to client
                $this->dispatch(new OnlinePaymentEmailJob($fundRequest, $client, $emailTemplateId = 35, $requestOnlinePayment, 'bank_reference_client', ($fundRequest) ? 'fundrequest' : 'fund' ));
