...
class HomeController extends BaseController
{
    ...
    public function loadInvoiceNotification(Request $request)
    {
        ...
        ->leftJoin('invoices','invoices.id','=','all_history.activity_for')
        ->leftJoin('users as u1','u1.id','=','all_history.client_id')
        ->select("users.*","all_history.*","case_master.case_title","case_master.id","task_activity.title","all_history.created_at as all_history_created_at","case_master.case_unique_number","invoices.deleted_at as deleteInvoice",DB::raw('CONCAT_WS(" ",u1.first_name,u1.middle_name,u1.last_name) as fullname'),"case_master.deleted_at as deleteCase","invoices.unique_invoice_number")
        ->where('all_history.is_for_client','no')
        ->where("all_history.firm_id",Auth::User()->firm_name)
                        ...
                        if(isset($ritem->user_popup_remind_detail)) {
                            ...
                            if($isExist && isset($isExist->snooze_remind_at)) {
                                ...
                                if(Carbon::parse($currentTime)->gte($remindTime) && Carbon::parse($eventStartTime)->gt(Carbon::parse($currentTime))) {
                                    ...
                                    $addEvent = false;
                                    Log::info("else time not matched: ");
                                }
                            } else {
