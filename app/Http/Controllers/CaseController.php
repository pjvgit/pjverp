...
//        $caseStageList = CaseStage::where("status","1")->get();
    ...
    public function showCaseDetails(Request $request)
    {
        ...
        $CaseMaster = CaseMaster::join('users','users.id','=','case_master.created_by')->select("case_master.*","case_master.id as case_id","users.id","users.first_name","users.last_name","users.user_level","users.email","case_master.created_at as case_created_date","case_master.created_by as case_created_by")
                        ->where("case_unique_number",$request->id)->where("case_master.firm_id", $authUser->firm_name);
    ...
    public function saveAddEventPage(Request $request)
    {
        ...
        if(!isset($request->recuring_event)){
            ...
            $startDate = strtotime(date("Y-m-d",  strtotime($request->start_date)));
            $endDate = strtotime(date("Y-m-d",  strtotime($request->end_date)));
            $recurringEndDate =  strtotime(date('Y-m-d',strtotime($request->start_date.' +365 days')));
            if($request->end_on!=''){
        ...
        if(!isset($request->recuring_event)) {
            if(isset($request->all_day)) {
                ...
                $start_date = $this->eventConvertTimestampToUtc($request->start_date, $request->start_time, $authUser->user_timezone, 'dateFromTime');
            ...
            $eventRecurring = EventRecurring::create([
                ...
                "start_date" => $start_date,
                "end_date" => $end_date,
                "event_reminders" => (in_array($authUser->id, $request['linked_staff_checked_share'])) ? $this->getUpdateEventReminderJson($caseEvent, $request) : Null,
        ...
        if($request->delete_event_type=='SINGLE_EVENT') {
            ...
            if($caseEvent && $caseEvent->is_recurring == 'no' && !isset($request->recuring_event)) {
                ...
                $recurringEvent = EventRecurring::create([
                    ...
                    "start_date" => $start_date,
                    "end_date" => $end_date,
                    'event_reminders' => (in_array($authUserId, $request['linked_staff_checked_share'])) ? $this->getUpdateEventReminderJson($caseEvent, $request) : Null,
                    'event_linked_staff' => $this->getEventLinkedStaffJson($caseEvent, $request),
                    'event_linked_contact_lead' => $this->getEventLinkedContactLeadJson($caseEvent, $request),
                ...
                if($caseEvent->edit_recurring_pattern == 'single event' && $caseEvent->parent_event_id != '') {
                    $caseEvent->fill([
                        ...
                        "start_time" => ($request->start_time && !isset($request->all_day)) ? $start_time : NULL,
                        "end_time" => ($request->end_time && !isset($request->all_day)) ? $end_time : NULL,
                        "recurring_event_end_date" => $recurring_end_date,
                        "is_full_day" => (isset($request->all_day)) ? "yes" : "no",
                        "event_description" => $request->description,
                        "is_recurring" => "yes",
                        "event_location_id" => $locationID ?? NULL,
                ...
                $caseEvent->fill([
                    ...
                    "yearly_frequency" => $request->yearly_frequency,
                    "is_no_end_date" => (isset($request->no_end_date_checkbox)) ? "yes" : "no",
                    "end_on" => (!isset($request->no_end_date_checkbox) && $request->end_on) ? $recurring_end_date : NULL,
                    "is_event_private" => (isset($request->is_event_private)) ? 'yes' : 'no',
                    "firm_id" => $authUser->firm_name,
                    "updated_by" => $authUser->id,
                ])->save();
                ...
                /* else if($request->event_frequency == "YEARLY") {
                    $recurringEvent = $this->saveYearlyRecurringEvent($caseEvent, $start_date, $request, $recurring_end_date);
                } */
                $this->saveEventRecentActivity($request, $caseEvent->id, @$recurringEvent->id);
                if(!isset($request->no_case_link) && $request->text_lead_id!='') {
                    $this->sendLeadUserInviteEmail($request->text_lead_id, $recurringEvent, $caseEvent);
            ...
            if($caseEvent->event_recurring_type != $request->event_frequency) {
                ...
                if($request->event_frequency == 'DAILY') { 
                    ...
                    if($oldEvent->start_date != $eventRecurring->start_date) {
                        ...
                        if($oldEvent->event_interval_day != $request->event_interval_day || $oldEvent->is_no_end_date != $isNoEndDate || isset($request->updated_start_date) || isset($request->updated_end_on)) {
                        ...
                        if($oldEvent->event_interval_day != $request->event_interval_day || $oldEvent->is_no_end_date != $isNoEndDate || isset($request->updated_start_date) || isset($request->updated_end_on)) {
                            EventRecurring::where("event_id", $oldEvent->id)->where("id", ">=", $request->event_recurring_id)->forceDelete();
                            $eventRecurring = $this->saveDailyRecurringEvent($oldEvent, $user_start_date, $request, $user_recurring_end_date);
                        } else {
                            ...
                            $this->updateFollowingRecurringEvent($request, 'daily', $user_start_date, $user_recurring_end_date, $oldEvent, null, $days);
                        } 
                    ...
                    if($oldEvent->start_date != $eventRecurring->start_date) {
                        ...
                        if($oldEvent->is_no_end_date != $isNoEndDate || isset($request->updated_start_date) || isset($request->updated_end_on)) {
                            EventRecurring::where("event_id", $oldEvent->id)->where("id", ">=", $request->event_recurring_id)->forceDelete();
                            $eventRecurring = $this->saveBusinessDayRecurringEvent($caseEvent, $user_start_date, $request, $user_recurring_end_date);
                        } else {
                            /* $eventLinkedStaff = $this->getEventLinkedStaffJson($caseEvent, $request);
                            $eventLinkedClient = $this->getEventLinkedContactLeadJson($caseEvent, $request);
                            $period = \Carbon\CarbonPeriod::create($start_date, $recurring_end_date);
                            ...
                            $this->updateFollowingRecurringEvent($request, 'business_day', $user_start_date, $user_recurring_end_date, $oldEvent, $caseEvent, $days);
                        }
                        if(!isset($request->no_case_link) && $request->text_lead_id!='') {
                            $this->sendLeadUserInviteEmail($request->text_lead_id, $eventRecurring, $caseEvent);
                        }  
                        ...
                        if($oldEvent->event_interval_day != $request->event_interval_day || $oldEvent->is_no_end_date != $isNoEndDate || isset($request->updated_start_date) || isset($request->updated_end_on)) {
                            EventRecurring::where("event_id", $oldEvent->id)->where("id", ">=", $request->event_recurring_id)->forceDelete();
                            $eventRecurring = $this->saveBusinessDayRecurringEvent($oldEvent, $user_start_date, $request, $user_recurring_end_date);
                        } else {
                            /* $eventLinkedStaff = $this->getEventLinkedStaffJson($oldEvent, $request);
                            $eventLinkedClient = $this->getEventLinkedContactLeadJson($oldEvent, $request);
                            $period = \Carbon\CarbonPeriod::create($start_date, $recurring_end_date);
                            ...
                            $this->updateFollowingRecurringEvent($request, 'business_day', $user_start_date, $user_recurring_end_date, $oldEvent, null, $days); 
                        }
                        $oldEvent->fill([
                            "event_title" => $request->event_name,
                            "case_id" => (!isset($request->no_case_link) && $request->text_case_id!='') ? $request->text_case_id : NULL,
                    ...
                    if($oldEvent->start_date != $eventRecurring->start_date) {
                        ...
                        if(array_diff( $request->custom, $oldEvent->custom_event_weekdays ) || $request->daily_weekname != $oldEvent->event_interval_week || $oldEvent->is_no_end_date != $isNoEndDate || isset($request->updated_start_date) || isset($request->updated_end_on)) {
                            ...
                            $this->updateFollowingRecurringEvent($request, 'custom', $user_start_date, $user_recurring_end_date, $oldEvent, $caseEvent, $days);
                            if(!isset($request->no_case_link) && $request->text_lead_id!='') {
                                $this->sendLeadUserInviteEmail($request->text_lead_id, $eventRecurring, $caseEvent);
                            }
                        }  
                        ...
                        if(array_diff( $request->custom, $oldEvent->custom_event_weekdays ) || $request->daily_weekname != $oldEvent->event_interval_week || $oldEvent->is_no_end_date != $isNoEndDate || isset($request->updated_start_date) || isset($request->updated_end_on)) {
                            ...
                            $this->updateFollowingRecurringEvent($request, 'custom', $user_start_date, $user_recurring_end_date, $oldEvent, null, $days);
                        }
                        ...
                        $oldEvent->fill([
                            ...
                            "start_time" => ($request->start_time && !isset($request->all_day)) ? $start_time : Null,
                            "end_time" => ($request->end_time && !isset($request->all_day)) ? $end_time : Null,
                            "recurring_event_end_date" => ($request->end_on) ? $recurring_end_date : $oldEvent->recurring_event_end_date,
                            "is_full_day" => (isset($request->all_day)) ? "yes" : "no",
                            "event_description" => $request->description,
                            "is_recurring" => "yes",
                            "event_location_id" => $locationID ?? NULL,
                    ...
                    if($oldEvent->start_date != $eventRecurring->start_date) {
                        ...
                        if(isset($request->updated_start_date) || $oldEvent->is_no_end_date != $isNoEndDate || isset($request->updated_end_on)) {
                            EventRecurring::where("event_id", $oldEvent->id)->where("id", ">=", $request->event_recurring_id)->forceDelete();
                            $eventRecurring = $this->saveWeeklyRecurringEvent($caseEvent, $user_start_date, $request, $user_recurring_end_date);
                        } else {
                            /* $eventLinkedStaff = $this->getEventLinkedStaffJson($caseEvent, $request);
                            $eventLinkedClient = $this->getEventLinkedContactLeadJson($caseEvent, $request);
                            ...
                            $this->updateFollowingRecurringEvent($request, 'weekly', $user_start_date, $user_recurring_end_date, $oldEvent, $caseEvent, $days);
                        }
                        if(!isset($request->no_case_link) && $request->text_lead_id!='') {
                            $this->sendLeadUserInviteEmail($request->text_lead_id, $eventRecurring, $caseEvent);
                        }
                        ...
                        if(isset($request->updated_start_date) || $oldEvent->is_no_end_date != $isNoEndDate || isset($request->updated_end_on)) {
                            EventRecurring::where("event_id", $oldEvent->id)->where("id", ">=", $request->event_recurring_id)->forceDelete();
                            $eventRecurring = $this->saveWeeklyRecurringEvent($oldEvent, $user_start_date, $request, $user_recurring_end_date);
                        } else {
                            /* $eventLinkedStaff = $this->getEventLinkedStaffJson($oldEvent, $request);
                            $eventLinkedClient = $this->getEventLinkedContactLeadJson($oldEvent, $request);
                            $period = \Carbon\CarbonPeriod::create($start_date, '7 days', $recurring_end_date);
                            ...
                            $this->updateFollowingRecurringEvent($request, 'weekly', $user_start_date, $user_recurring_end_date, $oldEvent, null, $days);
                        }
                        $oldEvent->fill([
                            "event_title" => $request->event_name,
                            "case_id" => (!isset($request->no_case_link) && $request->text_case_id!='') ? $request->text_case_id : NULL,
                    ...
                    if($oldEvent->start_date != $eventRecurring->start_date) {
                        ...
                        if($oldEvent->event_interval_month != $request->event_interval_month || $request->monthly_frequency != $oldEvent->monthly_frequency || $oldEvent->is_no_end_date != $isNoEndDate || isset($request->updated_start_date) || isset($request->updated_end_on)) {
                            EventRecurring::where("event_id", $oldEvent->id)->where("id", ">=", $request->event_recurring_id)->forceDelete();
                            $eventRecurring = $this->saveMonthlyRecurringEvent($caseEvent, $user_start_date, $request, $user_recurring_end_date);
                        } else {    
                            /* $eventLinkedStaff = $this->getEventLinkedStaffJson($caseEvent, $request);
                            $eventLinkedClient = $this->getEventLinkedContactLeadJson($caseEvent, $request);
                            ...
                            $this->updateFollowingRecurringEvent($request, 'monthly', $user_start_date, $user_recurring_end_date, $oldEvent, $caseEvent, $days);
                        }                    
                    } else {
                        if($oldEvent->event_interval_month != $request->event_interval_month || $request->monthly_frequency != $oldEvent->monthly_frequency || $oldEvent->is_no_end_date != $isNoEndDate || isset($request->updated_start_date) || isset($request->updated_end_on)) {
                            EventRecurring::where("event_id", $oldEvent->id)->where("id", ">=", $request->event_recurring_id)->forceDelete();
                            $eventRecurring = $this->saveMonthlyRecurringEvent($oldEvent, $user_start_date, $request, $user_recurring_end_date);
                        } else {
                            /* $eventLinkedStaff = $this->getEventLinkedStaffJson($oldEvent, $request);
                            $eventLinkedClient = $this->getEventLinkedContactLeadJson($oldEvent, $request);
                            $period = \Carbon\CarbonPeriod::create($start_date, $recurring_end_date);
                            ...
                            $this->updateFollowingRecurringEvent($request, 'monthly', $user_start_date, $user_recurring_end_date, $oldEvent, null, $days);
                        }
                        $oldEvent->fill([
                            "event_title" => $request->event_name,
                            "case_id" => (!isset($request->no_case_link) && $request->text_case_id!='') ? $request->text_case_id : NULL,
                ...
                /* else if($request->event_frequency == 'YEARLY') {
                    ...
                    if($oldEvent->start_date != $eventRecurring->start_date) {
                        ...
                        if($oldEvent->event_interval_year != $request->event_interval_year || $request->yearly_frequency != $oldEvent->yearly_frequency) {
                            EventRecurring::where("event_id", $oldEvent->id)->where("id", ">=", $request->event_recurring_id)->forceDelete();
                            $eventRecurring = $this->saveYearlyRecurringEvent($caseEvent, $start_date, $request, $recurring_end_date);
                        } else {    
                            $eventReminders = $this->getEventReminderJson($caseEvent, $request);
                            $eventLinkedStaff = $this->getEventLinkedStaffJson($caseEvent, $request);
                            $eventLinkedClient = $this->getEventLinkedContactLeadJson($caseEvent, $request);
                            $period = \Carbon\CarbonPeriod::create($start_date, $request->event_interval_year.' years', $recurring_end_date);
                            $days = $this->getDatesDiffDays($request);
                            foreach($period as $date) {       
                                $currentWeekDay = strtolower(date('l', strtotime($request->start_date))); 
                        ...
                        if($oldEvent->event_interval_year != $request->event_interval_year) {
                            EventRecurring::where("event_id", $oldEvent->id)->where("id", ">=", $request->event_recurring_id)->forceDelete();
                            $eventRecurring = $this->saveYearlyRecurringEvent($oldEvent, $start_date, $request, $recurring_end_date);
                        } else {
            ...
            if($caseEvent->event_recurring_type != $request->event_frequency) {
                ...
                if($request->event_frequency=='DAILY') {
                    ...
                    foreach($oldEvents as $ekey => $eitem) {
                        ...
                        $eitem->fill([
                            ...
                            "recurring_event_end_date" => ($request->updated_end_on) ? $recurring_end_date : $eitem->recurring_event_end_date,
                            "is_full_day" => (isset($request->all_day)) ? "yes" : "no",
                            ...
                            "is_no_end_date" => (isset($request->no_end_date_checkbox)) ? "yes" : "no",
                            "end_on" => (!isset($request->no_end_date_checkbox) && $request->updated_end_on) ? $recurring_end_date : $eitem->end_on,
                ...
                /* else if($request->event_frequency == 'YEARLY') {
                    $oldEvents = Event::whereId($request->event_id)->orWhere("parent_event_id", $request->event_id)->where("edit_recurring_pattern", "!=", "single event")->get();
                    foreach($oldEvents as $ekey => $eitem) {
                        if($request->yearly_frequency != $eitem->yearly_frequency || $request->event_interval_year != $eitem->event_interval_year || isset($request->updated_start_date) || isset($request->updated_end_on)) {
                            ...
                            $eventRecurring = $this->saveYearlyRecurringEvent($eitem, $sdate, $request, $recurring_end_date);
                        } else {    
                        ...
                        $eitem->fill([
                            ...
                            "start_time" => ($request->start_time && !isset($request->all_day)) ? $start_time : Null,
                            "end_time" => ($request->end_time && !isset($request->all_day)) ? $end_time : Null,
                            "recurring_event_end_date" => ($request->end_on) ? $recurring_end_date : $eitem->recurring_event_end_date,
                            "is_full_day" => (isset($request->all_day)) ? "yes" : "no",
                            "event_description" => $request->description,
                            "is_recurring" => "yes",
                            "event_location_id" => $locationID ?? NULL,
    ...
    public function loadEditEventPage(Request $request)
      {
            ...
            $eventReminderData = []; $isAuthUserLinked = '';
            if($eventRecurring) {
                $eventReminderData = encodeDecodeJson($eventRecurring->event_reminders)->where('created_by', auth()->id());
   ...
   public function deleteEventFromCommentPopup(Request $request)
   {
    ...
    public function saveEventReminderPopup(Request $request)
    {
        ...
        if($allRecurrings) {
            foreach($allRecurrings as $erkey => $eventRecurring) {
                $eventReminders = [];
