...
use App\CaseIntakeFormFieldsData,App\PotentialCaseInvoice,App\CaseEventLinkedContactLead;
...
class LeadController extends BaseController
{
    ...
    public function index()
    {
        ...
        foreach($LeadStatus as $k=>$v){
            ...
            $total = []; 
            $total = $allLeads->map(function($da) {
                return ($da->potential_case_value != "") ? (float) $da->potential_case_value : 0;
            });
            $extraInfo[$v->id]['sum'] = $total->sum();
    ...
    {       
        ...
        $loadFirmUser = User::select("first_name","last_name","id","parent_user")->where("firm_name",Auth::user()->firm_name)->where("user_level","3")->whereNotIn('id',$caseNoneLinkedStaffList)->where("user_status","1")->get();
       ...
       //Load Linked staff
        ...
        if(isset($task_id) && $task_id!=''){
            ...
            $caseNonLinkedAssigned = CaseTaskLinkedStaff::select("task_linked_staff.user_id")->where("linked_or_not_with_case","no")->where("task_linked_staff.is_assign","no")->where("task_linked_staff.is_contact","yes")->where("task_linked_staff.task_id",$task_id)->get()->pluck('user_id');
        ...
        // $caseNoneLinkedStaffList = CaseStaff::select("case_staff.user_id as case_staff_user_id")->where("case_id",$case_id)->get()->pluck('case_staff_user_id');
        // $loadFirmUser = User::select("first_name","last_name","id","parent_user")->where("firm_name",Auth::user()->firm_name)->where("user_level","3")->whereNotIn('id',$caseNoneLinkedStaffList)->get();
   ...
   {    
    ...
    public function loadAddEventPage(Request $request)
    {
      ...
      $caseLeadList = LeadAdditionalInfo::join('users','lead_additional_info.user_id','=','users.id')->select("first_name","last_name","users.id","user_level")->where("users.user_type","5")->where("users.user_level","5")->where("firm_name", Auth::user()->firm_name)->where("lead_additional_info.is_converted","no")->get();
   ...
   public function loadAllCaseStaffMember(Request $request)
   {
    ...
    public function loadEditEventPage(Request $request)
    {
          ...
          $getEventColorCode = EventType::select("color_code","id")->where('firm_id',Auth::User()->firm_name)->where('id',$evetData->event_type)->pluck('color_code');
   ...
   public function loadSingleEditEventPage(Request $request)
   {
      ...
      $country = Countries::get();
         ...
         $getEventColorCode = EventType::select("color_code","id")->where('id',$evetData->event_type)->where('firm_id',Auth::User()->firm_name)->pluck('color_code');
         $eventLocationAdded=[];
         if($evetData->event_location_id!=""){
   ...
   public function saveNonLinkedStaffToEvent($request,$event_id)
    ...
    {   
        $requestData= $_REQUEST;
        $FindInvoice = Invoices::select("*");      
        $FindInvoice = $FindInvoice->where("user_id",$requestData['user_id']);        
        $FindInvoice = $FindInvoice->where("is_lead_invoice","yes");        
        $totalData=$FindInvoice->count();
        $totalFiltered = $totalData; 
     
        $FindInvoice = $FindInvoice->offset($requestData['start'])->limit($requestData['length']);
        $FindInvoice = $FindInvoice->orderBy('created_at','DESC');
        $FindInvoice = $FindInvoice->get();
        $json_data = array(
            "draw"            => intval( $requestData['draw'] ),   
    ...
    public function addNewInvoices(Request $request)
    {
        ...
        $maxNumber=DB::table("invoices")->max("id") + 1;
    ...
    public function saveInvoices(Request $request)
    {
        ...
        $validator = \Validator::make($request->all(), [
            ...
            'invoice_number' => 'required|numeric|unique:invoices,id',
            'due_date' => 'required',
        ...
        if ($validator->fails())
        {
            ...
            DB::table('invoices')->where("deleted_at","!=",NULL)->where("id",$request->invoice_number)->delete();
            ...
            if($InvoiceSave->id >= 0){                
                ...
                if(empty($TaskActivity)){
                    $newTaskAvtivity = new TaskActivity;
                    $newTaskAvtivity->title="Consultation Fee";
                    $newTaskAvtivity->status="1";
                    $newTaskAvtivity->firm_id=Auth::User()->firm_name; 
                    $newTaskAvtivity->created_by=Auth::User()->id; 
                    $newTaskAvtivity->save();
                    $TaskAvtivityId = $newTaskAvtivity->id;
                }else{
                ...
                $TaskTimeEntry->entry_date=convertDateToUTCzone(date("Y-m-d", strtotime(date('Y-m-d',strtotime($request->invoice_date)))), auth()->user()->user_timezone ?? 'UTC'); 
                $TaskTimeEntry->entry_rate=str_replace(",","",$request->total_amount);
                $TaskTimeEntry->rate_type="flat";
                $TaskTimeEntry->duration=0;
    ...
    public function editInvoice(Request $request)
    {
        ...
        $userData=User::find($FindInvoice->user_id);
        return view('lead.details.case_detail.invoices.editInvoice',compact('userData','FindInvoice','invoice_id'));
    ...
    public function downloadInvoiceForm(Request $request)
    {
        ...
        $userData = User::select("users.*","countries.name as countryname")->leftJoin('lead_additional_info','users.id',"=","lead_additional_info.user_id")->leftJoin('countries','users.country',"=","countries.id")->where("users.id",$PotentialCaseInvoice['user_id'])->first();
        ...
        $path = asset(Storage::url("download/pdf/".$filename));
        return response()->json([ 'success' => true, "url"=>$path,"file_name"=>$filename], 200);
        exit;
    }
