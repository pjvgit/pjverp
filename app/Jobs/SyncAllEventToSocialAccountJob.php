...
class SyncAllEventToSocialAccountJob implements ShouldQueue
{
    ...
    {        
        ...
        if($syncAccount && $syncAccount->social_type == 'google') {
            $this->syncEventToGoogleCalendar($syncAccount);
        } else if($syncAccount && $syncAccount->social_account == 'outlook') {
            $this->syncEventToOutlookCalendar($syncAccount);
        } else {
    ...
    /**
     * Sync past 3 months events to google calendar if not synced
     */
    public function syncEventToGoogleCalendar($syncAccount)
    {
        ...
        foreach($allEvents as $item) {
            $event = $item;
            $checkIfExist = EventSyncToUserSocialAccount::where('user_id', $this->authUser->id)->where('event_id', $event->id)->where('social_type', $syncAccount->social_type)->first();
    ...
    /**
     * Sync past 3 months events to outlook calendar if not synced
     */
    public function syncEventToOutlookCalendar($syncAccount)
    {
        $dateS = Carbon::now()->startOfMonth()->subMonth(2);
        $dateE = Carbon::now();
        $eventIds = EventRecurring::whereDate("start_date", ">=", $dateS)->whereDate("start_date", "<=", $dateE)
