        ...
        <div class="history-item row ">
            ...
            <div class="history-info d-flex col-8">
                <img class="comment-avatar"
                    src="{{BASE_URL}}assets/images/faces/default_face.svg" width="32" height="32">
...
<?php 
 ...
 if(!empty($updatedEvenByUserData)){?>
<div class="history-item row ">
    <div class="history-date col-4">
        <p class="date-string">
            <?php 
            $CommonController= new App\Http\Controllers\CommonController();
            $updateDate=$CommonController->convertUTCToUserTime($evetData->updated_at,Auth::User()->user_timezone);?>
    ...
    <div class="history-info col-8">
        <span>Event updated by
            <a class=""
                href="{{BASE_URL}}contacts/attorneys/{{base64_encode($updatedEvenByUserData->id)}}">{{substr($updatedEvenByUserData->first_name,0,15)}}
                {{substr($updatedEvenByUserData->last_name,0,15)}}
                ({{$userTypes[$updatedEvenByUserData->user_type]}})
            </a>
        </span>
    </div>
</div>
