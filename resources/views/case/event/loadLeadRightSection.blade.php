            ...
            <tr class="sharing-user">
                <td class="d-flex  no-border "><span class="mr-2">{{$val->first_name}} {{$val->last_name}}</span>
                    <a class="event-name d-flex align-items-center" tabindex="0" role="button" href="javascript:;"
                        ...
                        </span><br><?php } ?> <?php if($val->email==''){?> No Email.</span> <br> <?php } ?> <a href='{{ route('case_details/info', $val->id) }}'>Edit Info</a>"
                        data-html="true">
                ...
                <td>
                    <label class="mb-0">
                        <input  class="lead_client_attend_all_users"  id="attend_user_{{$val->id}}" 
