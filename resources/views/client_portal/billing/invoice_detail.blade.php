		...
		<div class="payable-detail ">
			<div class="detail-view__header">
				...
				<div class="payable-detail__actions">
					...
					@if($onlinePaymentSetting && $onlinePaymentSetting->is_accept_online_payment == 'yes' && !in_array($invoice->status, ["Paid","Forwarded"]))
					<a class="btn btn-primary payable-detail__export-link ml-5" href="{{ route('client/bills/payment', ['type'=>'invoice', 'id'=>encodeDecodeId($invoice->id, 'encode'), 'client_id'=>encodeDecodeId(auth()->id(), 'encode')]) }}">
					...
					<div class="payable-detail__summary">
						...
						<div class="payable-detail__summary__right-panel">
							...
							<div class="payable-detail__datapair">
								...
								<div class="payable-detail__total-balance-due">${{ @$invoice->getInstallmentDueAmount() }}</div>
							</div>
							<div class="payable-detail__datapair">
						...
						<div id="payment_history">
							...
							<div class="list-row  ">
								...
								<div class="payable-detail-item-description">
									...
									@endif
									@if($item->acrtivity_title=="Payment Received" && $item->status == "6")
										<div class="position-relative" style="float: right;">
