            ...
            <td class="sorting_1" style="font-size: 13px;">
                <div class="text-left">
                    ...
                    <a class="name" href="{{ route('contacts/attorneys/info', base64_encode($v->user_id)) }}">{{$v->first_name}}
                        {{$v->last_name}} ({{$v->user_title}})</a> {{$v->activity}} <a class="name" href="{{BASE_URL}}event/view/{{base64_encode($v->event_id)}}"> {{$v->event_name}} </a> </a> <abbr
                        class="timeago" title="{{$v->all_history_created_at}}">about {{$v->time_ago}}</abbr> via web |
                      <?php  if($v->event_for_case!=NULL){  ?>  
            
                        {{-- <a class="name" href="{{BASE_URL}}court_cases/{{$v->events_for['case_unique_number']}}/info">{{$v->events_for['case_title']}}</a> --}}
                        <a class="name" href="{{ route('info', $v->events_for['case_unique_number']) }}">{{$v->events_for['case_title']}}</a><?php
                      }else if($v->event_for_lead!=NULL){
                        ?>  <a class="name" href="{{BASE_URL}}leads/{{$v->events_for['id']}}/case_details/info">{{$v->events_for['first_name']}}
