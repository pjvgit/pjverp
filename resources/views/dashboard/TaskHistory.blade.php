                ...
                ?>
                    ...
                    <a class="name" href="{{BASE_URL}}/contacts/attorneys/{{base64_encode($v->user_id)}}">{{$v->first_name}}
                      ...
                      <?php  if($v->task_for_case!=NULL){  ?>  
                        ...
                        <a class="name" href="{{BASE_URL}}court_cases/{{$v->task_for['case_unique_number']}}/info">{{$v->task_for['case_title']}}</a><?php
                      }else if($v->task_for_lead!=NULL){
                        ?>  <a class="name" href="{{BASE_URL}}leads/{{$v->task_for['id']}}/case_details/info">{{$v->task_for['first_name']}}
